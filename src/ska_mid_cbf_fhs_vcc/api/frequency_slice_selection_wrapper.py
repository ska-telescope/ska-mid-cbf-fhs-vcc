import logging

from ska_control_model import SimulationMode

from ska_mid_cbf_fhs_vcc.api.common.interfaces.fhs_base_api_interface import (
    FhsBaseApiInterface,
)
from ska_mid_cbf_fhs_vcc.api.emulator.frequency_slice_selection_emulator_api import (
    FrequencySliceSelectionEmulatorApi,
)
from ska_mid_cbf_fhs_vcc.api.simulator.frequency_slice_selection_simulator import (
    FrequencySliceSelectionSimulator,
)


class FrequencySliceSelectionApi(FhsBaseApiInterface):
    def __init__(
        self,
        device_id: str,
        config_location: str,
        simulation_mode: SimulationMode,
        emulation_mode: bool,
        logger: logging.Logger,
    ) -> None:
        self.logger = logger

        self.logger.info(f"Emulation mode={emulation_mode} : simulation mode={simulation_mode}")

        if simulation_mode == SimulationMode.TRUE:
            self.logger.info("instantiating simualator api")
            self._api = FrequencySliceSelectionSimulator(device_id, logger)
        elif simulation_mode == SimulationMode.FALSE and emulation_mode is True:
            self.logger.info("instantiating emulator api")
            self._api = FrequencySliceSelectionEmulatorApi(
                device_id=device_id, config_location=config_location, logger=logger
            )
        else:
            # TODO todo add FW API here no
            raise NotImplementedError("FW Api not implemented")

    def recover(self):
        return self._api.recover()

    def configure(self, config):
        return self._api.configure(config)

    def start(self) -> int:
        raise NotImplementedError("Frequency Slice Selection Start command not implemented")

    def stop(self, force: bool = False):
        raise NotImplementedError("Frequency Slice Selection Stop command not implemented")

    def deconfigure(self, config):
        return self._api.deconfigure(config)

    def status(self, status, clear: bool = False):
        return self._api.status(status, clear)
